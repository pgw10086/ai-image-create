...
export interface SmartLayoutSettings {
  ...
  enableTwoStageGeneration: boolean; // 两阶段生成：先构图草稿再精修（默认关闭；仅显式存 true 才开启）
}
...
*   **工作区高度（修正）**：智能布局画布模块高度应随 `canvasSize.height` 变化，确保画布为主体；当画布过大时允许滚动查看。
...
*   **工具栏自适应（修正）**：当窗口较窄导致按钮过多时，应将“画布尺寸/商品主体/渲染风格/高级开关/历史入口”等配置项收纳到下拉菜单，保证主操作始终可见。
*   **左侧常驻工具栏（新增，方案C）**：在画布工作区左侧增加纵向工具栏，承载“模板与填充”入口，降低二级菜单依赖。\n    *   必含入口：模板库、保存模板、从商品原型生成画布（解析模板）、从商品设计图复刻布局（生成模板）。\n    *   以图标按钮为主，悬停 Tooltip 说明。\n    *   专注模式下同样可见且可用，不占用画布内容区。\n    *   生成中禁用“从商品原型生成画布/从商品设计图复刻布局”，避免并发写入画布结构。
    *   从商品图生成模板弹窗需包含“意图卡”区（图型/平台尺寸/密度/风格/文案结构），并可回显与随模板一起保存。
...
**3.4 右侧面板 (Side Panel)**
*   右侧面板为可收起/展开的固定区域，用于承载“区域配置”和“文案变量”两类信息（Tab 切换）。
*   在“区域”Tab 内：仅当 `selectedZoneId` 不为空时显示配置表单；未选中时展示可点击的 Zone 列表。
*   **国际化**：所有界面文本（标题、按钮、标签等）均使用中文。
*   包含字段：
*   *   提示词描述 (TextArea): 支持多行输入；在标题右侧提供“优化”按钮，一键优化当前区域提示词并支持撤销。
...
*   **生成结果历史记录（必须）**：每次生成应写入一条“历史记录”并持久化到 IndexedDB（兼容从 localStorage 迁移）。\n    *   默认最多保留 50 条，超过后按时间从旧到新淘汰。\n    *   记录内容：生成时间、请求张数、每张图的状态（生成中/成功/失败/已停止）与结果 URL（若有）。\n    *   刷新页面后应可恢复最近的历史记录（至少不丢失已生成成功的结果）。
...
*   **默认样例模板（内置）**：应用启动时会自动导入/合并默认模板库（位于 `app/src/default-smart-layout-templates/`），用于提供可直接套用的样例画布。\n    *   默认模板 JSON 更新后应无需手动清理 localStorage 即可看到最新模板；合并时不覆盖用户模板，必要时以“（内置更新）”新增条目的形式提供最新版本。\n    *   当前内置样例（节选）：丝带、家居（香薰蜡烛/抱枕/桌面收纳盒/陶瓷花瓶）、发饰（抓夹/发箍/大肠发圈/珍珠一字夹套装）。\n    *   能力展示型复杂模板（节选）：\n        *   遮挡叠层×多参考图｜双主体+四特写（斜切背景）\n        *   遮挡叠层×多参考图｜三主体阶梯台座+四特写\n        *   遮挡叠层×多参考图｜右侧主舞台+左栏四特写\n        *   遮挡叠层×多参考图｜双主体错位叠放+环绕四特写\n    *   默认样例为控制本地存储体积，除个别历史模板外通常不携带 `snapshotDataUrl`；用户可在画布中打开并“保存当前为模板”以生成本地预览缩略图。
