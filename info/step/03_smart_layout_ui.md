...
*   **结构化布局描述面板（必须）**：在右侧面板提供 Tab“布局描述（给模型看的）”，用于预览最终会写入 Prompt 的结构化段落，并支持复制。\n    *   `GLOBAL_PROMPT`\n    *   `REGION_PROMPTS`（每个 Zone 一条）\n    *   `DEPTH_TREE`\n    *   `规则声明`：例如“不要在最终图片里画任何边框/编号/文字；对象必须严格放在各自区域内”。\n    描述内容由 Step 4 的组装函数实时生成并显示，面板内可折叠展示。
    *   **提示词优化（新增）**：在“布局描述”面板提供按钮“优化提示词”，点击后对所有 Zone 的 `prompt` 做一次自动优化，并回填到各 Zone 的提示词中；提供“撤销”以回退本次优化。\n        *   调用方式：前端直连 GLM-5 对话补全接口。\n        *   配置项：`VITE_BIGMODEL_API_KEY`（必填），可选 `VITE_BIGMODEL_API_URL`（默认 `https://open.bigmodel.cn/api/paas/v4/chat/completions`）。\n        *   注意：前端直连会暴露 Key，仅建议用于本地/内网调试环境。
*   **交互逻辑**：
