...
export async function parseSmartLayoutTemplateFromImage(input: {
  ...
  const system = [
    ...
    '- prompt 必须自洽可复用：不得引用输入图编号或“如图所示/参照第N张图”等表述（禁止 image_0/image_1、图1/图2）。',
    '- 模板需要可复用：不要把具体商品写死在 prompt 中。对于主商品/玩具/主体，请统一使用占位符 {PRODUCT}，让用户后续替换。',
    '- 若涉及项目/主题/活动（PROJECT），仅允许一句抽象描述（可用 {PROJECT}），不要展开具体元素清单与长段落。',
...
export async function generateSmartLayoutTemplateFromProductImage(input: {
  ...
  const system = [
    ...
    '- 不要在输出 JSON 中主动新增 PROJECT 或 copyVariables.PROJECT；仅当效果描述明确给出项目名时，才可在 zones.prompt 中使用 {PROJECT}，且仅一句抽象描述。',
